	    ...
	    if(is_array($sidebars)){
			foreach($sidebars as $sidebar){
				...
				register_sidebar(array(
					...
					'id' => 'thefox_'.strtolower($sidebar_class),
					'before_widget' => '<div class="sb_widget clearfix %2$s">',
					'after_widget' => '</div>',
					'before_title' => '<h3>',
					'after_title' => '</h3>',
		    	));
...
public static		function save_form($post_id){
		$is_saving = isset($_POST['sbg_edit']);
		if(!empty($is_saving)){
...
public static	function edit_form(){
	...
	<div id='sbg-sortables' class='meta-box-sortables us_box_ctn'>
