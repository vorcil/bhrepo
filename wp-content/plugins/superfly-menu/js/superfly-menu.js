...
//						setTimeout(function() {dragging = false}, 0);
					...
					$sidebar.find('.sf-rollback .sf-navicon-button').add('#sf-mob-navbar .sf-navicon-button').on(clickTapEvent, function(){
					...
					if ($t.is('.sf-has-child-menu')) {
						...
						if ( maxWidth < _ww ) {
							...
							if (!$sub.is('.sf-submenu-visible')) {
								$sub.parent().find('.sf-submenu-visible').removeClass('sf-submenu-visible').slideUp();
